...
import { cellText, findHeader, readWorkbook, type Sheet } from "@core/xlsx/reader";
import { localPath, readLocalBytes } from "./helpers/local";

function sheet(name: string, rows: (string | number | null)[][], merges: string[] = [], pictures: { row: number; col: number }[] = []): Sheet {
...
const prospectionSheet = sheet("Prospe TEST", [
  [],
